...
{% for node in object.nodeset_set.all %}

  var group = Number({{ node.group }})
  var newNodesobj = { label: "{{ node.name }}", id: {{ forloop.counter }}, group: {{ node.group }}, reflexive: false };
  nodes.push(newNodesobj);
{% endfor %}

  

{% for link in object.network_set.all %}     
   var newLinkObj = { id: {{ forloop.counter }}, source: {{ link.sourceID }}, target: {{ link.targetID }}, weight: {{ link.weight }}, cost: {{ link.weight }}};
